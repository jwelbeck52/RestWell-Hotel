...
class RoomSelectionScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    var roomData = Provider.of<Rooms>(context);
    var myRooms = roomData.rooms;

    return SafeArea(
      child: Scaffold(
        appBar: MainAppBar("Room Selection"),
        body: ListView.builder(
            itemCount: myRooms.length,
            itemBuilder: (context, index) {
              return ListTile(
                  title: Text(myRooms[index].id +
                      ' - ' +
                      myRooms[index].name +
                      ' - ' +
                      myRooms[index].cost.toString()));
            }),
      ),
    );
  }
